...
class core_admin_renderer extends plugin_renderer_base {
     ...
     */
    public function plugins_check(array $plugininfo) {
        ...
        $this->page->requires->js_init_call('M.core_plugin.init_plugins_check');
        $this->page->requires->string_for_js('showall', 'core_plugin');
        $this->page->requires->string_for_js('showhighlighted', 'core_plugin');
        ...
        $numofimportant = array();    // number of important rows per this subsection
        ...
        foreach ($plugininfo as $type => $plugins) {
            ...
            foreach ($plugins as $name => $plugin) {
                ...
                if ($isstandard and in_array($statuscode, array(plugin_manager::PLUGIN_STATUS_NODB, plugin_manager::PLUGIN_STATUS_UPTODATE))) {
                    $row->attributes['class'] .= ' unimportant';
                } else {
                    $numofimportant[$type]++;
                }
        ...
        $out  = html_writer::tag('div', get_string('loading', 'core_admin') . ' ' . $this->pix_icon('i/progressbar', '',
                'core', array('class' => 'progressbar')), array('id' => 'collapsecontroller'));
    ...
    public function plugins_control_panel(array $plugininfo) {
        ...
        foreach ($plugininfo as $type => $plugins) {
            ...
            if (empty($plugins)) {
                ...
                $msg->colspan = count($table->head);
                $row = new html_table_row(array($msg));
                $row->attributes['class'] .= ' noneinstalled';
                $table->data[] = $row;
                continue;
            }
