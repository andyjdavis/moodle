...
function message_get_recent_conversations($user, $limitfrom=0, $limitto=100) {
    ...
    $sql = "SELECT $userfields, mr.id as mid, mr.smallmessage, mr.fullmessage, mr.timecreated, mc.id as contactlistid, mc.blocked
              ...
              JOIN (
                  ...
                  GROUP BY messages.uid
                   ) messages2 ON mr.id = messages2.mid AND (mr.useridto = messages2.uid OR mr.useridfrom = messages2.uid)
              JOIN {user} u ON u.id = messages2.uid
    ...
    $sql = "SELECT $userfields, m.id as mid, m.smallmessage, m.fullmessage, m.timecreated, mc.id as contactlistid, mc.blocked
              ...
              JOIN (
                  ...
                  GROUP BY messages.uid
                   ) messages2 ON m.id = messages2.mid AND (m.useridto = messages2.uid OR m.useridfrom = messages2.uid)
              JOIN {user} u ON u.id = messages2.uid
         LEFT JOIN {message_contacts} mc ON mc.userid = :userid3 AND mc.contactid = u.id
             WHERE u.deleted = '0'
             ORDER BY m.id DESC";
    ...
    //union the 2 result sets together looking for the message with the most recent timecreated
    //$conversation->id is the other user's ID
