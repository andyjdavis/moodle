...
class backup_module_structure_step extends backup_structure_step {
    ...
    protected function define_structure() {
        ...
        // attach outcome support plugin structure to $module element, only one allowed
        $this->add_plugin_structure('outcomesupport', $module, false);
...
/**
 * This structure step is responsible for backing up used outcome sets
 * in the course and outcome marks and outcome marks history.
 */
class backup_course_outcome_structure_step extends backup_structure_step {
    /**
     * Prevent execution if outcomes is disabled
     *
     * @return bool
     */
    ...
    protected function define_structure() {
        $root      = new backup_nested_element('course_outcome');
        $usedsets  = new backup_nested_element('outcome_used_sets');
        $usedset   = new backup_nested_element('outcome_used_set', array('id'), array('idnumber', 'filter'));
        $marks     = new backup_nested_element('outcome_marks');
        $mark      = new backup_nested_element('outcome_mark', array('id'), array(
            'idnumber', 'userid', 'graderid', 'result', 'timecreated', 'timemodified',
        ));
        $histories = new backup_nested_element('outcome_marks_histories');
        $history   = new backup_nested_element('outcome_marks_history', array('id'), array(
            'action', 'outcomemarkid', 'idnumber', 'userid', 'graderid', 'result', 'timecreated',
        ));

        $root->add_child($usedsets);
        $root->add_child($marks);
        $usedsets->add_child($usedset);
        $marks->add_child($mark);
        $mark->add_child($histories);
        ...
        if ($this->get_setting_value('users')) {
            $mark->set_source_sql('
                SELECT m.*, o.idnumber
                  FROM {outcome} o
            INNER JOIN {outcome_marks} m ON o.id = m.outcomeid
                 WHERE m.courseid = ?
            ', array(backup::VAR_COURSEID));

            $history->set_source_table('outcome_marks_history', array('outcomemarkid' => backup::VAR_PARENTID));
        }

        $mark->annotate_ids('user', 'userid');
        $mark->annotate_ids('user', 'graderid');
        $history->annotate_ids('user', 'userid');
        $history->annotate_ids('user', 'graderid');
...
class backup_questions_structure_step extends backup_structure_step {
    ...
    protected function define_structure() {
        ...
        // attach outcome support plugin structure to $question element, only one allowed
        $this->add_plugin_structure('outcomesupport', $question, false);
...
/**
 * Structure step in charge of constructing the outcome.xml file for all the
 * outcome areas that are used by the activity and the attempts generated by
 * those areas.
 */
class backup_activity_outcomes_structure_step extends backup_structure_step {
    /**
     * Prevent execution if outcomes is disabled
     *
     * @return bool
    ...
    protected function define_structure() {
        $usedareas = new backup_nested_element('outcome_used_areas');
        $usedarea  = new backup_nested_element('outcome_used_area', array('id'), array('cmid', 'outcomeareaid'));
        $attempts  = new backup_nested_element('outcome_attempts');
        $attempt   = new backup_nested_element('outcome_attempt', array('id'), array(
            'outcomeusedareaid', 'userid', 'itemid', 'percentgrade', 'mingrade', 'maxgrade',
            'rawgrade', 'timecreated', 'timemodified', 'component', 'area',
        ));

        $usedareas->add_child($usedarea);
        $usedarea->add_child($attempts);
        $attempts->add_child($attempt);

        $usedarea->set_source_table('outcome_used_areas', array(
            'cmid' => backup_helper::is_sqlparam($this->get_task()->get_moduleid())
        ));

        if ($this->get_setting_value('userinfo')) {
            $attempt->set_source_sql('
               SELECT a.*, area.area, area.component
